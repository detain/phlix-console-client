...
final class SpikeTest extends TestCase
{
    ...
    public function testVideoFrameDecodesWithAutoMode(): void
    {
        if (!$this->haveFfmpeg()) {
            self::markTestSkipped('ffmpeg not available');
        }
        ...
        // Test with 'auto' mode - exercises the RendererFactory::autoMode() branch
        $frames = (new VideoSpike())->frames($this->mp4, 1, 40, 12, 'auto');

        self::assertCount(1, $frames);
        self::assertNotSame('', $frames[0]);
    }

    public function testVideoFrameDecodesWithNullMode(): void
    {
        if (!$this->haveFfmpeg()) {
            self::markTestSkipped('ffmpeg not available');
        }
        ...
        // Test with null mode - exercises the RendererFactory::autoMode() branch
        $frames = (new VideoSpike())->frames($this->mp4, 1, 40, 12, null);
