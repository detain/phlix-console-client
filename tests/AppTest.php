...
final class AppTest extends TestCase
{
    ...
    public function testCtrlKOpensThePaletteWithStaticActionsAndFetchesLibraries(): void
    {
        ...
        self::assertSame(['Search', 'Home', 'Settings', 'Stats', 'Watch History', 'Favorites', 'Photos', 'Music Artists', 'Playlists', 'Collections', 'Servers', 'Shared With Me', 'Invite Links', 'Federation Shares', 'Show metrics', 'Log out', 'Quit'], $labels);
    }
    ...
    public function testOpenAdminSectionUsersPushesTheUsersScreenWithAFetch(): void
    {
        [$adminApp] = $this->appWithAdminUser(true);
        [$admin] = $adminApp->update(new OpenAdminMsg());

        [$users, $cmd] = $admin->update(new OpenAdminSectionMsg(Route::AdminUsers));

        self::assertSame(Route::AdminUsers, $users->route());
    ...
    public function testOpenAdminSectionBackupPushesTheBackupScreenWithAFetch(): void
    {
        ...
        [$backup, $cmd] = $admin->update(new OpenAdminSectionMsg(Route::AdminBackup));

        self::assertSame(Route::AdminBackup, $backup->route());
    ...
    public function testOpenAdminSectionSettingsPushesTheSettingsScreenWithAFetch(): void
    {
        ...
        [$settings, $cmd] = $admin->update(new OpenAdminSectionMsg(Route::AdminSettings));

        self::assertSame(Route::AdminSettings, $settings->route());
        self::assertInstanceOf(AdminSettingsScreen::class, $settings->screen());
        self::assertSame(3, $settings->stackDepth(), 'the settings screen is pushed onto the admin menu');
        self::assertInstanceOf(\Closure::class, $cmd, 'the settings screen fetches on push');
    }

    public function testOpenAdminSectionDlnaPushesTheDlnaScreenWithAFetch(): void
    {
        [$adminApp] = $this->appWithAdminUser(true);
        [$admin] = $adminApp->update(new OpenAdminMsg());

        [$dlna, $cmd] = $admin->update(new OpenAdminSectionMsg(Route::AdminDlna));

        self::assertSame(Route::AdminDlna, $dlna->route());
