 ...
 *
 * Mirrors the FIXED server contract shared by three endpoints:
 *   - `POST /api/v1/media/{id}/transcode`      → `variants: Rendition[]|null`
 *   - `GET  /api/v1/transcode/{jobId}/status`  → `variants: Rendition[]|null`
 *   - `GET  /api/v1/media/{id}/playback`       → `quality_ladder: Rendition[]|null`
 *
 * `id` is the fixed lowercase ladder token (`240p`,`360p`,`480p`,`720p`,`1080p`,
 * `1440p`,`2160p`,`original`). `url` is a SIGNED path to that variant's own
