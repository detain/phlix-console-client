...
/**
 * The admin Filesystem Browser: a navigable listing of the server's configured
 * browse roots and their subdirectories (`GET /api/v1/admin/fs/browse`).
 *
 * Opens on the ROOTS view: an empty-path request makes the server list its
 * configured browse roots (its `data.path` is null in that view; the screen
 * shows its own '/' header until the first real directory is entered). The
 * rail serves DIRECTORIES ONLY — it never returns files or the
 * `type`/`size`/`modified` metadata the pre-S405 client parsed, so every row
 * is mapped dir-only client-side. Jail semantics are the server's: outside
 * roots 403, unknown paths 404, non-directories 400 — all centralised into
 * failures by the client's decode seam.
 *
